...
async function bootstrap() {
  ...
  try {
    ...
    // E2E test for the EA Sync and Tournaments flow
    // Modified to verify husky strict commit
    ...
    for (const ta of teamAwards) {
      console.log(
        `  - Team ${ta.teamId} received Award '${ta.award.name}' with text: '${ta.awardedForText}'`,
      );
    }
    ...
    const userAwards = await prisma.userAward.findMany({
      where: {
        userId: { in: teams.flatMap((t) => [t.ownerId, ...t.members.map((m) => m.userId)]) },
      },
      include: { award: true },
    });
